...
**外殼**：`enclosure/` 同步改為 v1.4：前殼上牆開 6 × 4.2 的快門窗（Z 0.2–4.4），T 形快門帽凸緣 7.6 × 4.0 × 0.8 在牆內、頭 5.6 × 3.6 穿牆露出 1 mm，桿頭到牆內面 1.06 mm 所以預行程 0.26 mm；後殼唇口在同位置開缺口。BOOT / RESET 改成 Ø2 針孔，原本的長條凸台與三個圓按鈕帽取消。螢幕模組的固定：後殼底板長四支 Ø4 定位柱到模組 PCB 朝底板那一面（離板頂 13.84 mm），頂端 Ø1.8 凸柱穿進模組四角 Ø2 孔，窗框內側貼 0.5 mm 泡棉膠，模組不再只靠 J5 母座懸著。2026-09-23 把相機模組（含排線反折）、電池與電池線、記憶卡、螢幕模組也依文件尺寸建成簡化零件放進組合件，干涉 0 件；相機鏡座頂 6.9 mm 離原本 7 mm 的天花板只剩 0.1，前殼內高改 8 mm（整組 28.6 mm）。2026-09-23 外觀修飾：直角圓角 R3→R4.5，兩面外緣整圈 R2.5 倒圓，鏡頭孔外加 Ø16 凸 1.5 mm 的鏡座環，螢幕窗外 28.5 × 28.5 內縮 0.8 mm 邊框，鏡頭面左側六道握持溝紋。組合件干涉檢查 0 件（含用簡化 STEP 補上的 SW3 本體）。**2026-09-23 晚上改成螢幕用七條公–母杜邦線接 J5**（省事、不焊；母座腳序與模組一致，一對一插）：模組不再插母座，移到 x 10.4–49.6（中心 x 30，排針那排朝左 x 12.9），用 M2×4 鎖在後殼四支 Ø4.5 矮柱上；後殼內深 14.5 → 27.0 mm（杜邦公頭膠殼 14 mm 站在 8.5 mm 的母座上，再留 3.5 mm 讓線彎出來），整機 40.1 mm；外殼螺絲 M2×25 → M2×35；螢幕窗中心 (40.5, 45) → (28.4, 45)。杜邦線建成 `offboard-display-cable` 放進組合件，干涉 0 件。
...
> 2026-09-23 改用杜邦線接法後模組移到 x 10.4–49.6、排針朝左，顯示區中心 **(28.4, 45)**，四角孔 (12.9, 33.6)、(47.1, 33.6)、(12.9, 56.4)、(47.1, 56.4)；見下方與 2d 節。
底面高度（直插版）：母座 8.5 + 模組自帶排針塑膠 2.54 = 模組 PCB 背面離板底 **11.0 mm**，再加模組 PCB 1.2 與玻璃 1.6，亮面約 **13.9 mm**；電池 6 + 泡棉 2 = 8 mm，到模組背面還有 3 mm。
底面高度（**杜邦線版，2026-09-23 起採用**）：J5 上的公頭膠殼到板底下 22.5，線彎出來 3.5，外殼內深 27.0；模組背面 23.6、亮面 26.4，電池到模組背面之間 15.6 mm 就是走線的空間。
...
機構（v1.3 原案）：螢幕在底面朝外（與鏡頭相反面），模組自帶的 7 pin 直排針插進 J5 母座，身體蓋在電池上方，沒有任何線。

> **2026-09-23 改為杜邦線**：模組排針不插母座，改用七條公–母杜邦線（公頭插 J5、母頭套模組排針，腳序一對一，GND 對 GND）。理由是省事、不用焊。模組移到 x 10.4–49.6、排針那排朝左（x 12.9，離 J5 遠一點才有空間繞線），四角用 M2×4 鎖在後殼矮柱上；杜邦膠殼 14 mm 讓外殼後半加深到 27 mm 內深（整機 40.1 mm）。買到腳序相反的相容品也不用改板，線交叉接即可。細節與走線在 `enclosure/README.md`。
...
底面高度（直插版）：母座 8.5 + 模組排針塑膠 2.54 → 模組 PCB 背面離板底 11.0 mm，亮面約 13.9 mm。杜邦線版：模組背面 23.6、亮面 26.4，外殼底側內深 27.0。想壓低可換 5 mm 高的矮母座（模組背面降到 7.5 mm，就要改用 5 mm 厚的 503040 電池 + 1 mm 泡棉）。
