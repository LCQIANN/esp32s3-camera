...
| 螢幕 | J5 1×7 2.54 mm 母座（底面右板邊），通用型 1.3 吋 IPS 240×240 ST7789 7 pin 模組直插；SCL=GPIO44、SDA=GPIO43、RES=GPIO6、DC=GPIO5、BLK=GPIO4。UART0 排針已取消 |
...
- **預覽螢幕直插板上母座 J5**，沒有杜邦線；模組蓋在電池上方，亮面離板底約 13.9 mm。細節與外殼用的尺寸在 `DESIGN.md` 「v1.3 改了什麼」與 2d 節。
...
| 檔案 | 給誰看 | 內容 |
|---|---|---|
| `esp32s3-camera-v1.2-簡易版.pptx` / `.pdf` | 外行人、家人朋友、沒碰過電路板的人 | 9 頁，不用零件編號與專業縮寫：它能做什麼、正反面長什麼樣、工廠做哪些我們做哪些、要買什麼、三步組裝、三件注意事項 |
| `esp32s3-camera-v1.2.pptx` / `.pdf` | 自己下單與組裝時對照、有電子背景的人 | 17 頁完整版：依 PCB 座標畫的零件分工圖、Economic / Standard 差異、58 顆 SMD 清單、3D 渲染、組裝尺寸與高度、含購買連結的採購表 |

另有一個線上頁面「ESP32-S3 相機板零件分工圖」（claude.ai 私人連結，組織內可看），2026-09-22 已更新到 v1.4：依座標畫的零件分工圖、組裝示意與裸板 3D 渲染、外殼渲染、採購清單。

兩份都用 PowerPoint 匯出成 PDF，PDF 內的購買連結可點。**兩份簡報仍是 v1.2 內容**：與 v1.4 的差別是 J5/J6 兩條排針變成一個 1×7 螢幕母座（螢幕不用杜邦線）、快門 SW3 換成側按式移到上板邊，其餘零件與位置相同。
...
| USB-C 線 | 要能傳資料 | J1 |
| 預覽螢幕 | 通用型 1.3 吋 IPS 240×240 ST7789，7 pin 2.54 mm 直排針已焊，27.78 × 39.22 mm | 直插 J5 |
...
- **底面高度**：電池 + 泡棉 8 mm；螢幕模組 PCB 背面離板底 11.0 mm（母座 8.5 + 模組排針塑膠 2.54），亮面約 13.9 mm，外殼底側內高抓 ≥ 14.5 mm。
- **外殼開孔**：鏡頭 (30, 25) 頂面；快門上板邊 x 47.6–53.6、板頂以上 0.2–4.4 mm；BOOT/RESET 針孔 (9, 24)、(9, 32)；開關右板邊 y 8–14；USB-C 右板邊 y 19.5–28.5；記憶卡左板邊 y 36–52；螢幕窗 23.4 × 23.4 mm，中心約 (40.5, 45) 底面。板子 STEP 在 `esp32s3-camera.step`，含母座；螢幕模組的簡化模型在 `render/illustration/models/display_module.wrl`。

## 外殼

`enclosure/` 是給 v1.4 板子的相機式兩片殼（前殼鏡頭面、後殼螢幕面，快門帽在上牆），用 `enclosure/build_enclosure_sw.py` 透過 SolidWorks 2024 的 API 依板檔座標自動建模，組合件含板子 STEP，干涉檢查 0 件。螢幕模組由後殼四支定位柱插進其四角孔定位，快門帽在上牆。外形 65 × 90 × 27.6 mm，四支 M2×25 從背面鎖。尺寸依據、列印方向、組裝步驗與第一次實體要驗證的項目都在 `enclosure/README.md`。
